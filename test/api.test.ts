...
test('local API rejects cross-origin, unauthenticated and malformed requests, and serves real repositories', async () => {
  ...
  expect(await (await post('/preferences', { implementer: 'claude' })).json()).toEqual({ implementer: 'claude' });
...
test('preview routes return the recorded preview, serve its screenshots, and retake on request', async () => {
  ...
  const runner = { recovery: [], active: null, retakePreview: async (id: string) => { retaken.push(id); return store.require(id); } };
  const { server, base } = serveApi({ store, runner: runner as unknown as ApiServices['runner'] });
  onCleanup(async () => {
    server.stop(true);
    store.close();
    await rm(root, { recursive: true, force: true });
